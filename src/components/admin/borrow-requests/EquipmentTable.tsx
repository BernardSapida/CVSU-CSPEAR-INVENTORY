...
const EquipmentTable: FunctionComponent<EquipmentTableProps> = ({ columns, requestsList, conditionOptions, borrowStatusOptions }) => {
    ...
    const filteredItems = useMemo(() => {
        let filteredItems = [...requestsList];

        if (hasSearchFilter) {
            filteredItems = filteredItems.filter((equipment) =>
                equipment.name.toLowerCase().includes(filterValue.toLowerCase()),
            );
        }
        if (borrowStatusFilter !== "all" && Array.from(borrowStatusFilter).length !== borrowStatusOptions.length) {
            filteredItems = filteredItems.filter((equipment) =>
                Array.from(borrowStatusFilter).includes(equipment.borrow_status),
            );
        }
        if (conditionStatusFilter !== "all" && Array.from(conditionStatusFilter).length !== borrowStatusOptions.length) {
            filteredItems = filteredItems.filter((equipment) =>
                Array.from(conditionStatusFilter).includes(equipment.condition),
            );
        }

        return filteredItems;
    }, [requestsList, filterValue, borrowStatusFilter, conditionStatusFilter]);
        ...
        return (
            <div className="flex flex-col gap-4">
                <div className="flex justify-between gap-3 items-end">
                    ...
                    <div className="flex gap-3">
                        <Dropdown>
                            ...
                            >
                                {borrowStatusOptions.map((status) => (
                ...
                <div className="flex justify-between items-center">
                    <span className="text-default-400 text-small">Total {requestsList.length} requests</span>
    ...
    return (
        <>
            ...
            >
                ...
                <TableBody emptyContent={"No requestsList found"} items={sortedItems}>
                    {(item) => (
